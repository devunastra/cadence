...
import { createLeadView, deleteLeadView, updateLeadView, fetchLeadsPage, fetchLeadById, deleteLeads, bulkUpdateLeads, updateLead, saveUserPreferences, addStudioFieldOption, renameStudioFieldOption, deleteStudioFieldOption, logLeadActivity, updateStudioFieldOptionColor, savePageFilters } from '@/app/actions'
...
export function LeadsTable({ initialCustomViews, studioId, studioName, initialFieldOptions, initialLeads, initialTotal, initialPrefs, initialPageFilters }: LeadsTableProps) {
  ...
  // Debounced DB save when colWidths change
  useEffect(() => {
    if (Object.keys(colWidths).length === 0 || !studioId || !mounted) return
  ...
  // Persist filter + sort changes to Supabase (debounced 1s)
  useEffect(() => {
    if (!studioId || !mounted) return
